...
var reducers = {
	list: {
		...
		addMany: function () {
			for (var i = 0; i < state.menu.addManyCount; i++) reducers.list.addItem()
			up.batch(state.menu)
		},
		flush: function (e) {
			state.items = []
	...
	state: {
		...
		animate: function (e) {
			if (reducers.__animateInterval) {
				clearInterval(reducers.__animateInterval)
				reducers.__animateInterval = null
			} else {
				reducers.__animateInterval = setInterval(function () {
					var index;
					var ref;
